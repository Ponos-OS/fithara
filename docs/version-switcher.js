...
/*
 * Version switcher injected into each versioned Redoc page.
 ...
 */
(function () {
  const CURRENT_VERSION = (window.FITHARA_DOCS_VERSION || '').trim();
  ...
  function render(versions) {
    ...
    container.id = 'fithara-version-switcher';

    const label = document.createElement('label');
    label.textContent = 'version';
    label.setAttribute('for', 'fithara-version-select');
    container.appendChild(label);

    const select = document.createElement('select');
    select.id = 'fithara-version-select';
    versions.forEach(v => {
