...
2. Build only that step's scope. If an AC item is a human/organizational action (legal sign-off, security sign-off, someone approving something) rather than an engineering deliverable, don't fabricate a checkmark for it — build everything that *is* engineering work, then report the sign-off item's real current state plainly (Step B7 surfaced this: legal/security sign-off on canonical texts and disclaimers).
