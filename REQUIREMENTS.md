...
- `docs/<version>/openapi.json` and `docs/latest/openapi.json` are present alongside the rendered HTML for every published version (§5.4/§5.5) — not just the always-current `docs/openapi.json` at the repo root.
...
- Copies `docs/openapi.json` alongside the rendered HTML into both `docs/<version>/openapi.json` and (via the `docs/latest/` refresh) `docs/latest/openapi.json` — mirroring exactly how `smart-novel-beatrice`'s `docs.yml` ships `docs/schema.graphql` into `docs/<version>/schema.graphql`/`docs/latest/schema.graphql` alongside its SpectaQL output. This is what lets an integrator fetch the raw schema for a specific past release (e.g. to regenerate a client SDK) rather than only ever seeing the current `docs/openapi.json` at the repo root.
...
- Each `docs/<version>/` (and `docs/latest/`) directory carries its own copy of `openapi.json` alongside the rendered HTML — see §5.4 — the same way `smart-novel-beatrice` ships `schema.graphql` per version, not just the rendered docs site.
