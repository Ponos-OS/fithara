...
│   ├── index.html                     # Hand-written landing page for the GitHub Pages docs site; lists published versions
│   ├── versions.json                  # Generated by the docs-publish workflow; newest-first list of published version dirs
│   ├── version-switcher.js            # Injected into each versioned docs page so readers can jump between versions
│   └── .nojekyll                      # Tells GitHub Pages to serve the generated HTML/JS as-is, no Jekyll processing
├── .github/workflows/
│   ├── dockerhub-release.yml          # On push to main: gate on semantic-release, run tests, publish the Docker image
│   └── docs.yml                       # After a successful release: render docs/openapi.json into a versioned docs site
├── Dockerfile                         # Backend image (Part 5)
├── package.json, package-lock.json    # Node deps for the release toolchain (semantic-release + plugins) only — the app itself has no Node dependency
├── .releaserc.json                    # semantic-release config: commit-analyzer rules, pyproject.toml version bump, Docker publish
...
15. **Barrel imports are enforced, not just conventional** — via [`import-linter`](https://import-linter.readthedocs.io/), not a hand-rolled `ruff` rule (an earlier version of this used `ruff`'s `flake8-tidy-imports` banned-api table, one entry per submodule plus a matching per-file-ignore exemption per file that legitimately needed to bypass it; that grew linearly with every new file and wasn't solving an import-architecture problem with an import-architecture tool, so it was replaced). Importing a symbol from the submodule that defines it instead of the package barrel (`from src.utils.config import Settings` instead of `from src.utils import Settings`) fails `make lint_check` / `uv run lint-imports`, configured in `pyproject.toml`'s `[tool.importlinter]` section: one `protected` contract per barrel package, e.g.
    ```toml
    [[tool.importlinter.contracts]]
    name = "src.utils internals are private"
    type = "protected"
    protected_modules = ["src.utils.**"]
    allowed_importers = ["src.utils", "src.utils.**"]
    ```
    `**` matches at any depth (`*` matches only one level — see [import-linter's wildcard docs](https://import-linter.readthedocs.io/en/stable/contract_types/)), so `allowed_importers` including the package's own wildcard covers the barrel itself, sibling submodules importing each other, and colocated `*__test.py` files reaching for a private symbol — all three cases fall out of the same two-line contract, no separate exemption needed for any of them. Adding a new file to an *existing* barrel package needs **zero** config changes here. Adding a brand new barrel package needs exactly one new `protected` contract block (5 lines).
...
26. `CamelModel` sets `str_strip_whitespace=True` — every wire model's `str` fields get leading/trailing whitespace stripped before any other validation runs (this is Pydantic v2's built-in equivalent of a NestJS `@Transform(trim)`/class-transformer decorator, applied once for every model instead of per-field). This is why `min_length=1` alone is enough to reject a whitespace-only value like `"   "`: stripping happens first, so it's evaluated as length 0. Don't add a per-field `field_validator`/`BeforeValidator` to strip a wire-model string — it already happens.
27. When an optional parameter defaults to a real (possibly expensive-to-construct, possibly under-configured-in-tests) singleton — `registry: T | None = None` resolved via `registry if registry is not None else get_registry()` — resolve it as late as possible, immediately before the line that actually uses it, not unconditionally at the top of the function. Resolving it eagerly means every code path pays for it, including ones that return early and never touch it — `run_draft_agent`'s retry-exhausted-fallback path doesn't need the registry at all, but an eager `get_registry()` at the top made that path call `get_settings()` anyway and crash in any test that hadn't configured `LLM__*`.
28. **Never log `str(exc)` or a `ModelHTTPError.body` for an LLM provider failure.** A provider's own HTTP error body can echo back request details verbatim — confirmed against a real OpenAI 401, whose error body repeats the (here fake) API key back in plain text. Extract only structured, provider-agnostic fields instead: `type(exc).__name__`, and for `ModelHTTPError` specifically, `.status_code`/`.model_name`. This is a real secret-leak vector, not a theoretical one, and no unit test catches it unless it deliberately puts a fake secret in a mocked error body and asserts the logged fields never contain it.
29. `setup_observability()`/anything else that needs `get_settings()` at app-construction time belongs in `main.py`'s `lifespan` (startup), not in `create_app()`'s body directly — same reasoning as `get_registry()`'s existing placement there: `make schema` imports the module-level `app` object via a bare `from src.main import app`, with no env vars set, and that import must keep succeeding. Anything wired directly into `create_app()` that needs `Settings` breaks it immediately.
30. `semantic-release --dry-run` (used to validate `.releaserc.json`/`package.json` before trusting a CI job to run it) still needs a `GITHUB_TOKEN` in the environment even though dry-run mode skips every actual write (tag, publish, commit) — without one, plugin verification fails before it gets far enough to print the "next release version" line the CI job's own decision logic greps for. `GITHUB_TOKEN="$(gh auth token)" npm run --silent release -- --dry-run` is a safe way to smoke-test the config locally against the real repo: read-only, no side effects, but it does hit the real GitHub API so it needs real (if minimally-scoped) auth.
